...
# Single-flight gate so poll_asa + up-notify (and overlapping callers) share one CDN fetch.
...
def refresh_asa_cache(*, force: bool = False) -> AsaSnapshot:
    ...
    # Serialize CDN fetches: concurrent pollers wait, then reuse the fresh snapshot.
    with _refresh_gate:
        with _lock:
            if not force and snapshot_is_fresh(_snapshot):
                return _snapshot  # type: ignore[return-value]
        ...
        if (
            ...
            and snapshot.server_count != prev_good.server_count
        ):
            logger.info(
